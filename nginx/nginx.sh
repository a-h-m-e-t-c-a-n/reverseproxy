...
for domain in $DOMAINS; do
  if [ ! -f "/etc/nginx/ssl/domains/$domain/fullchain.pem" ]; then
    echo "Generating domains ceritificate for $domain"
    mkdir -p "/etc/nginx/ssl/domains/$domain"
    printf "[dn]\nCN=${domain}\n[req]\ndistinguished_name = dn\n[EXT]\nsubjectAltName=DNS:${domain}\nkeyUsage=digitalSignature\nextendedKeyUsage=serverAuth" > openssl.cnf
    openssl req -x509 -out "/etc/nginx/ssl/domains/$domain/fullchain.pem" -keyout "/etc/nginx/ssl/domains/$domain/privkey.pem" \
...
use_lets_encrypt_certificates() {
  ...
  sed -i "s|/etc/nginx/ssl/domains/$1|/etc/letsencrypt/live/$1|g" /etc/nginx/domains/*.conf
}
